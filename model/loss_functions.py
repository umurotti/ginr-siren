...
def KL_regularization_loss(weights : torch.Tensor, target_mean, target_std):
    # This function calculates the KL div. loss for weights wrt. Normal distr. with 0 mean and unit std.
    mean = weights.mean()
    std = weights.std()
    logvar = torch.log(std**2)
    
    def kl_divergence_normal(mean, logvar):
        return 0.5 * torch.sum(logvar.exp() - logvar - 1 + mean.pow(2))
    
    return kl_divergence_normal(mean, logvar)